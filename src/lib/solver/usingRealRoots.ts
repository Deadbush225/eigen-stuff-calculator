...
function optimizedNewtonRaphson(poly: string, coeffs: number[]): number[] {
	...
	for (let rootIndex = 0; rootIndex < degree; rootIndex++) {
		...
		const startingPoints = generateSmartStartingPointsOptimized(
			deflatedCoeffs,
			bounds,
			roots
		);
		...
		for (const start of startingPoints) {
			...
			if (root !== null) {
				...
				if (Math.abs(verification) < 1e-10) {
					...
					const { deflated, newCoeffs } = deflatePolynomial(
						deflatedCoeffs,
						root
					);
 ...
 */
function generateSmartStartingPointsOptimized(
	coeffs: number[],
 ...
 */
function robustNewtonIteration(
	polyString: string,
	start: number,
	existingRoots: number[]
): number | null {
 ...
 */
function deflatePolynomial(
	coeffs: number[],
	root: number
): { deflated: string; newCoeffs: number[] } {
...
function solveCubicOptimized(coeffs: number[]): number[] {
	...
	if (discriminant > 0) {
		...
		const u =
			Math.sign(-q / 2 + sqrt_disc) *
			Math.pow(Math.abs(-q / 2 + sqrt_disc), 1 / 3);
		const v =
			Math.sign(-q / 2 - sqrt_disc) *
			Math.pow(Math.abs(-q / 2 - sqrt_disc), 1 / 3);
