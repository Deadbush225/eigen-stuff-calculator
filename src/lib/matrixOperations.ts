...
function E3(
	...
	for (let j = 0; j < newMatrix[r2].length; j++) {
		if (
			...
			newMatrix[r2][
				j
			] = `(${newMatrix[r2][j]}) - (${newMatrix[r1][j]}) * ${scalar}`;
		}
	}
...
export function matrixRank(matrix: MatrixWString): number {
	...
	for (let row = 0; row < rrefMatrix.length; row++) {
		...
		for (let col = 0; col < rrefMatrix[row].length; col++) {
			if (
				typeof rrefMatrix[row][col] === "number" &&
				rrefMatrix[row][col] === 1
			) {
				rank++;
				break;
			}
		...
		for (let col = 0; col < rrefMatrix[row].length; col++) {
			if (
				...
				for (let prevCol = 0; prevCol < col; prevCol++) {
					if (
						typeof rrefMatrix[row][prevCol] === "number" &&
						rrefMatrix[row][prevCol] !== 0
					) {
						isLeading = false;
						break;
					}
...
export function findNullSpace(matrix: Matrix): BasisVector {
	...
	if (freeVars.length === 0) {
		console.warn(
			"No free variables found - this shouldn't happen for eigenspaces"
		);
		...
		if (pivotCol === -1) {
			for (let nextRow = row + 1; nextRow < matrix.length; nextRow++) {
				for (let col = 0; col < matrix[nextRow].length; col++) {
					if (
						typeof matrix[nextRow][col] === "number" &&
	...
	for (let row = 0; row < matrix.length; row++) {
		...
		if (pivotCol !== -1) {
			...
			for (let otherRow = 0; otherRow < matrix.length; otherRow++) {
				if (
					otherRow !== row &&
					typeof matrix[otherRow][pivotCol] === "number" &&
					matrix[otherRow][pivotCol] !== 0
				) {
					return false;
				}
			}
...
export function solveLinearSystem(A: number[][], b: number[]): number[] | null {
	...
	for (let row = 0; row < rows; row++) {
		...
		for (let col = 0; col < cols; col++) {
			if (
				typeof rrefAugmented[row][col] === "number" &&
				rrefAugmented[row][col] !== 0
			) {
				allZeroCoeffs = false;
				break;
			}
		...
		if (
			allZeroCoeffs &&
			typeof rrefAugmented[row][cols] === "number" &&
			rrefAugmented[row][cols] !== 0
		) {
	...
	const pivotCols = getPivotColumns(
		rrefAugmented.map((row) => row.slice(0, cols))
	);
...
export function testRREF() {
	...
	console.log(
		"Solution:",
		solution ? `[${solution.join(", ")}]` : "No unique solution"
	);
}
...
export function calculateDeterminant(matrix: number[][]): number {
	...
	for (let j = 0; j < n; j++) {
		...
		const cofactor =
			Math.pow(-1, j) * matrix[0][j] * calculateDeterminant(minor);
		det += cofactor;
	}
