...
const Tour: React.FC<TourProps> = ({ startTour, onTourComplete }) => {
  const tourInstanceRef = useRef<ReturnType<typeof introJs.tour> | null>(null);
  const retryTimeoutRef = useRef<number | null>(null);
  const isActiveRef = useRef(false);

  useEffect(() => {
    // Clean up any existing tour when component unmounts or startTour changes
    return () => {
  ...
  useEffect(() => {
    if (!startTour || isActiveRef.current) return;
    ...
    const checkAndStartTour = () => {
      ...
      if (retryCount >= maxRetries) {
        console.warn("Tour: Max retries exceeded, starting with available elements");
      }
      ...
      if (existingElements.length >= requiredElements.length || retryCount >= maxRetries) {
        // All required elements exist (or max retries reached), start the tour
        ...
        tourInstanceRef.current = introJs.tour();
        ...
        tourInstanceRef.current.setOptions({
          steps: steps,
          showProgress: true,
          showBullets: false,
        ...
        tourInstanceRef.current.oncomplete(() => {
          isActiveRef.current = false;
          tourInstanceRef.current = null;
          onTourComplete();
        });

        tourInstanceRef.current.onexit(() => {
          isActiveRef.current = false;
          tourInstanceRef.current = null;
          onTourComplete();
        });
        ...
        if (retryCount < maxRetries) {
          retryTimeoutRef.current = window.setTimeout(checkAndStartTour, 200);
        } else {
          isActiveRef.current = false;
        }
      }
    ...
    retryTimeoutRef.current = window.setTimeout(checkAndStartTour, 100);
