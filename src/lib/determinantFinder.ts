 ...
 */
export function calculateTriangularDeterminant(
 ...
 */
export function calculate2x2Determinant(
	matrix: (string | number)[][]
): LatexString {
 ...
 */
export function assembleNbyNDeterminantExpression(
	matrix: (string | number)[][],
	depth: number = 0
): LatexString {
	...
	const openBracket =
		depth === 0 ? "\\Biggl[" : depth === 1 ? "\\biggl[" : "\\Bigl[";
	const closeBracket =
		depth === 0 ? "\\Biggr]" : depth === 1 ? "\\biggr]" : "\\Bigr]";
	...
	for (let j = 0; j < n; j++) {
		...
		const term = `+(-1)^{(${1}+${
			j + 1
		})}(${element})${openBracket}${minorDet}${closeBracket}`;
		terms.push(term);
	}
